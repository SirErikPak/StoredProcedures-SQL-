if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[P_DDRT_Restricted]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[P_DDRT_Restricted]
GO
CREATE	PROCEDURE	P_DDRT_Restricted
AS
/*********************************************************************/
/*       WYETH CLOSURE – DDRT Awardees                               */
/*********************************************************************/
...
BEGIN	TRAN	LTIDOCDDRT	-- short-code of process-name
...
SET	dist_dollars =	(@CashValue * oustanding_shares),
	dist_shares  = (@ShareValue * oustanding_shares),
...
/*********************************************************************/
-- STEP-07 -- Determine Trust or GECS or Override
SELECT	@step = '07a--Deemed Deferrals Restricted for GECS'
...
/*********************************************************************/
SELECT	@step = '07b--Deemed Deferrals Restricted for SAP'
...
/*********************************************************************/
SELECT	@step = '07c--Deemed Deferrals Restricted for Override'
...
/*********************************************************************/
-- STEP-08 -- Unknown
SELECT	@step = '08-- Can Not Determine - Deemed Deferrals Restricted Trust'
...
IF	@FAIL = 'Y'
 Begin
	ROLLBACK TRAN	LTIDOCDDRT
	INSERT	WYETHaux.dbo.LTIDOCProcessLog
	VALUES 	(Getdate(), 'Close-' + @Process,3,'FAILED',@Process + ' FAILED AT STEP : '+ @step)
	RAISERROR (51103,16,1)   -- Custom SQL error-message in MASTER
	RETURN 16
 End
ELSE
 Begin
	COMMIT	TRAN	LTIDOCDDRT
