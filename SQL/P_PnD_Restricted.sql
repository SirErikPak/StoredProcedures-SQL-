if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[P_PnD_Restricted]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[P_PnD_Restricted]
GO
CREATE	PROCEDURE	P_PnD_Restricted
AS
/*********************************************************************/
/*       WYETH CLOSURE – PnD Awardees                                */
/*********************************************************************/
...
BEGIN	TRAN	LTIDOCPnD	-- short-code of process-name
...
SELECT	@step = '07a--PSAs non-deferred GECS'
...
Where	LTI.Process_id = @Process
And	RTRIM(OPT.Address6) IN (SELECT RTRIM(GLOBAL_ID) FROM WYETHaux.dbo.tblGECSemployee Where GLOBAL_ID IS NOT NULL)

SELECT 	@ERR = @@Error                   IF 	@ERR <> 0      GOTO   STEP99	--> --> --> --> --> --> --> --> --> --> --> --> --> 

/*********************************************************************/
SELECT	@step = '07a--PSAs non-deferred SAP'
...
ON	LTI.opt_num = OPT.opt_num
Where	LTI.Process_id = @Process
And	SUBSTRING(OPT.Address6,4,9) IN (SELECT GlobalID FROM WYETHaux.dbo.tblSAPimportUSA Where GlobalID IS NOT NULL)

SELECT 	@ERR = @@Error                   IF 	@ERR <> 0      GOTO   STEP99	--> --> --> --> --> --> --> --> --> --> --> --> --> 

/*********************************************************************/
SELECT	@step = '07a--PSAs non-deferred Override'
...
/*********************************************************************/
-- STEP-08 -- Unknown
SELECT	@step = '08-- Can Not Determine - PSAs non-deferred'
