if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[P_162M_Restricted]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[P_162M_Restricted]
GO
CREATE	PROCEDURE	P_162M_Restricted
AS
/*********************************************************************/
/*       WYETH CLOSURE – 162M Awardees                               */
/*********************************************************************/
...
BEGIN	TRAN	LTIDOC162M	-- short-code of process-name
...
SET	dist_dollars =	@CashValue * (@CloseFactor * oustanding_shares),
	dist_shares = (@CloseFactor * oustanding_shares),
	adjustment_shares = (1 - @CloseFactor) * oustanding_shares,
...
/*********************************************************************/
SELECT	@step = '07c--162M for Override'
...
/*********************************************************************/
SELECT	@step = '07d--162M for Trust Override '

UPDATE	wyethaux.dbo.LTIDOC_OutstandingAwards
SET	destination = @Trust
From	wyethaux.dbo.LTIDOC_OutstandingAwards
Where	Process_id = @Process
And	grant_num IN
 (
	Select	GrantNumber From tblgrantdefferals
 )
...
/*********************************************************************/
-- STEP-07 -- Trust
SELECT	@step = '07a--Paid out to SAP (80%)'
...
/*********************************************************************/
SELECT	@step = '07b--Paid out to New Trust (80%)'
...
/*********************************************************************/
-- STEP-08 -- Unknown
SELECT	@step = '08-- Can Not Determine - Paid out to New Trust (80%)'
