if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[P_ODRP_Restricted]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[P_ODRP_Restricted]
GO
CREATE	PROCEDURE	P_ODRP_Restricted
AS
/*********************************************************************/
/*       WYETH CLOSURE – ODRP Awardees                               */
/*********************************************************************/
...
BEGIN	TRAN	LTIDOCODRP	-- short-code of process-name
...
SELECT	@step = '07a--Outstanding Deferred RSUs and PSAs for GECS'
...
/*********************************************************************/
SELECT	@step = '07b--Outstanding Deferred RSUs and PSAs for Trust'
...
SELECT 	@ERR = @@Error                   IF 	@ERR <> 0      GOTO   STEP99	--> --> --> --> --> --> --> --> --> --> --> --> --> 
/*********************************************************************/
SELECT	@step = '07c--Outstanding Deferred RSUs and PSAs for Override'
...
/*********************************************************************/
-- STEP-08 -- Unknown
SELECT	@step = '08-- Can Not Determine - Outstanding Deferred RSUs and PSAs'
...
IF	@FAIL = 'Y'
 Begin
	ROLLBACK TRAN	LTIDOCODRP
	INSERT	WYETHaux.dbo.LTIDOCProcessLog
	VALUES 	(Getdate(), 'Close-' + @Process,3,'FAILED',@Process + ' FAILED AT STEP : '+ @step)
	RAISERROR (51104,16,1)   -- Custom SQL error-message in MASTER
	RETURN 16
 End
ELSE
 Begin
	COMMIT	TRAN	LTIDOCODRP
